...
struct GiantTextView: View {
    ...
    private var adjustedAvailableSize: CGSize {
        // On iPhone, when the view is rotated 90 degrees, we need to swap width and height
    ...
    func calculateOptimalFontSize() {
        ...
        let animationScaleFactor: CGFloat = selectedAnimation != .none ? 1.4 : 1.0
        let safetyMarginWidth: CGFloat = selectedAnimation != .none ? 0.20 : 0.10
        let safetyMarginHeight: CGFloat = selectedAnimation != .none ? 0.20 : 0.10
        ...
        while maxFontSize - minFontSize > precision {
            ...
            // Calculate how many lines would be used with this font size
            ...
            // Calculate line height (font size + some spacing)
            let lineHeight = testFontSize * 1.2 // 20% extra for line spacing
            let totalHeight = lineHeight * numberOfLines

            // Calculate the width of the longest line
            let charWidth = testFontSize * 0.6
            var maxLineWidth: CGFloat = 0
            for line in lines {
                let lineWidth = CGFloat(line.count) * charWidth + CGFloat(max(0, line.count - 1)) * CGFloat(kerning)
                maxLineWidth = max(maxLineWidth, lineWidth)
            }

            // Apply animation scaling if needed
            let finalWidth = maxLineWidth * animationScaleFactor
            let finalHeight = totalHeight * animationScaleFactor
    ...
    private func createNoTextFont() -> Font {
